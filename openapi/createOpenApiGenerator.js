...
var getPipeTarget = function (schema) {
...
var unwrapTransforms = function (schema) {
    var typeName = getTypeName(schema);
    var pipeTarget = getPipeTarget(schema);
    if (pipeTarget) {
        return unwrapTransforms(pipeTarget);
    }
    if (typeName === "object") {
        var shape_1 = getShape(schema);
        if (shape_1) {
            var newShape = Object.keys(shape_1).reduce(function (acc, key) {
                var _a;
                return (__assign(__assign({}, acc), (_a = {}, _a[key] = unwrapTransforms(shape_1[key]), _a)));
            }, {});
    ...
    if (typeName === "array") {
        var def = getDef(schema);
        if (def === null || def === void 0 ? void 0 : def.element) {
            return zod_1.z.array(unwrapTransforms(def.element));
        }
    }
    if (typeName === "optional") {
        var def = getDef(schema);
        if (def === null || def === void 0 ? void 0 : def.innerType) {
...
var zodToOpenApi = function (schema) {
    ...
    try {
        var unwrapped = unwrapTransforms(schema);
        var result = zod_1.z.toJSONSchema(unwrapped);
...
var shouldSkipParameterExtraction = function (schema) {
    ...
    if (typeName === "discriminatedUnion" || typeName === "union") {
        ...
        return options.every(function (option) {
            return shouldSkipParameterExtraction(option);
        });
    }
    return true;
};
...
var mergeUnionParameters = function (options, paramKind, extraDocumentation) {
    var e_1, _a, e_2, _b;
    if (options.length === 0)
        return [];
    var allOptionParams = options.map(function (option) {
        return zodObjectToParameters(option, paramKind, extraDocumentation);
    });
    var paramPresence = new Map();
    try {
        for (var allOptionParams_1 = __values(allOptionParams), allOptionParams_1_1 = allOptionParams_1.next(); !allOptionParams_1_1.done; allOptionParams_1_1 = allOptionParams_1.next()) {
            var optionParams = allOptionParams_1_1.value;
            try {
                for (var optionParams_1 = (e_2 = void 0, __values(optionParams)), optionParams_1_1 = optionParams_1.next(); !optionParams_1_1.done; optionParams_1_1 = optionParams_1.next()) {
                    var param = optionParams_1_1.value;
                    var existing = paramPresence.get(param.name);
                    if (existing) {
                        existing.count++;
                        if (!param.required)
                            existing.param.required = false;
                    }
                    else {
                        paramPresence.set(param.name, { count: 1, param: __assign({}, param) });
                    }
                }
            }
                ...
                try {
                    if (optionParams_1_1 && !optionParams_1_1.done && (_b = optionParams_1.return)) _b.call(optionParams_1);
                }
        ...
        try {
            if (allOptionParams_1_1 && !allOptionParams_1_1.done && (_a = allOptionParams_1.return)) _a.call(allOptionParams_1);
        }
        finally { if (e_1) throw e_1.error; }
    }
    return Array.from(paramPresence.values()).map(function (_a) {
        var count = _a.count, param = _a.param;
        return (__assign(__assign({}, param), { required: param.required && count === options.length }));
    });
};
var zodObjectToParameters = function (schema, paramKind, extraDocumentation) {
    ...
    return Object.keys(shape).map(function (paramName) {
        ...
        var unwrapped = unwrapTransforms(paramSchema);
        var initialTypeName = getTypeName(unwrapped);
