...
>(
	...
	openApiRootDoc: Omit<OpenAPI.Document, "paths" | "components"> & {
		components?: OmitFromExisting<
			OpenAPI.ComponentsObject,
			"securitySchemes"
		> & {
		...
		[Tag in keyof SharedRoutesByTag]: {
			...
			> & {
				extraDocs: {
					...
					urlParams?: PathParameters<
						SharedRoutesByTag[Tag][R]["url"]
					...
					responses: {
						[S in keyof SharedRoutesByTag[Tag][R]["responses"] &
							...
							WithExampleOrExamples<
								StandardSchemaV1.Infer<
									SharedRoutesByTag[Tag][R]["responses"][S]
								>
							>;
					};
				};
...
const throwIfNotZodSchema = (schema: StandardSchemaV1<any>): z.Schema<any> => {
	if (!(schema instanceof ZodType))
		throw new Error(
			"Only support Zod schemas are supported for OpenAPI generation",
		);
		...
		paths: keys(sharedRoutesByTag).reduce((rootAcc, tag) => {
			...
			return {
				...
				...keys(sharedRoutes).reduce((acc, routeName) => {
					...
					const queryParamsZodSchema = throwIfNotZodSchema(
					...
					const { withRequestBodyExemple, requestBodyDocs } =
						extractFromOpenApiBody(extraDocs?.body);

					const requestBodySchema = throwIfNotZodSchema(
						...
						[formattedUrl]: {
							...
							[route.method]: {
								...
								responses: keys(route.responses).reduce((acc, status) => {
									const responseZodSchema = throwIfNotZodSchema(
...
const isShapeObjectEmpty = <T>(schema: z.Schema<T>): boolean => {
	...
	console.log("################# isShapeObjectEmpty", schema);
	if (typeName === "object") {
...
const getTypeName = <T>(schema: z.Schema<T>) => schema.def.type;
