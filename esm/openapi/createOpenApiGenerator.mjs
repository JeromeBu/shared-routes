...
export var createOpenApiGenerator = function (sharedRoutesByTag, openApiRootDoc) { return function (extraDataByRoute) { return (__assign(__assign({}, openApiRootDoc), { paths: keys(sharedRoutesByTag).reduce(function (rootAcc, tag) {
        ...
        return __assign(__assign({}, rootAcc), keys(sharedRoutes).reduce(function (acc, routeName) {
            ...
            var parameters = __spreadArray(__spreadArray(__spreadArray([], __read((pathParams.length > 0 ? pathParams : [])), false), __read((!isShapeObjectEmpty(queryParamsZodSchema)
                ? zodObjectToParameters(queryParamsZodSchema, "query", extraDocs === null || extraDocs === void 0 ? void 0 : extraDocs.queryParams)
                : [])), false), __read((!isShapeObjectEmpty(headerZodSchema)
                ? zodObjectToParameters(headerZodSchema, "header", extraDocs === null || extraDocs === void 0 ? void 0 : extraDocs.headerParams)
                : [])), false);
